...
std::vector<Ray> Camera::Cast(){
    ...
    Eigen::Vector4f start(0.0f, 0.0f, this->offset, 0.0f);
    ...
    for(auto pixelY = 0; pixelY < this->height; pixelY += 1){
        for(auto pixelX = 0; pixelX < this->width; pixelX += 1){
            ...
            Eigen::Vector4f through(startX+(pixelX*this->screenToWorldRatio), startY-(pixelY*this->screenToWorldRatio), 1.0f, 0.0f);
